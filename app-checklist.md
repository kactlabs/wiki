/ [Home](index.md)

## App Checklist

- Monolithic / Modular App
- Regular Registration
- Regular Login
- Gmail Registration
- Gmail Login
- Email with Providers
- Google Analytics
- KWebtics - [https://github.com/kactlabs/kweb-analytics](https://github.com/kactlabs/kweb-analytics)
- BYOK - For only GenAI related apps
- MongoDB CRUD Framework
- LLM.py
- Blog - [https://github.com/kactlabs/simple-blog-manager](https://github.com/kactlabs/simple-blog-manager)
- Newsletter
- Admin Login
- Dev DB to Prod DB Migration script
- Terms and Conditions (ToC)
- Privacy
- Testimonials
  - Video
  - Audio
  - Text
  - HandWritten
- New Feature with .env control